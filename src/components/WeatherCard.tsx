...
export default function WeatherCard({ coords }: { coords: { lat: number; lon: number } }) {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        if (!json.weather) throw new Error("Datos incompletos");
  ...
  return (
    <div className={`p-4 border bg-black rounded bg-opacity-10 backdrop-blur-5`}>
      <div className="row align-items-center">
        <div className="col-md-6 text-center text-md-start mb-3 mb-md-0">
          ...
          <div className="d-flex align-items-center justify-content-center justify-content-md-start mt-3">
            ...
            />
            <div>
              <p className={`display-3 mb-0 fw-bold`}>{data.weather.temp}°C</p>
            ...
            <div className="row g-2">
              <div className="col-6">
                <div className="p-2">
                  ...
                  <p className={`mb-0 h5`}>{data.weather.feels_like}°C</p>
                </div>
              </div>
              <div className="col-6">
                <div className="p-2">
                  <p className={`mb-1 small`}>Humedad</p>
                  <p className={`mb-0 h5`}>{data.weather.humidity}%</p>
                </div>
              </div>
              <div className="col-6">
                <div className="p-2">
                  <p className={`mb-1 small`}>Viento</p>
                  <p className={`mb-0 h5`}>{data.weather.wind || '--'} km/h</p>
                </div>
              </div>
              <div className="col-6">
                <div className="p-2">
                  <p className={`mb-1 small`}>Presión</p>
                  <p className={`mb-0 h5`}>{data.weather.pressure || '--'} hPa</p>
                </div>
              </div>
            </div>
          </div>
